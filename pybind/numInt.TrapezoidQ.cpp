...
double trapezoid_quadrature_01(
  std::string function,   /**< Funkcja przyjmująca i przekazująca
                                 parametr typu double.               */
  double step_size                  ///< Długość pojedynczego podziału
){
    double evaluation = 0;
    double x = 0;
    ...
    parser_t parser;
    if (!parser.compile(function, expression)) throw std::exception();

    x = 1;
    double val_in_1 = expression.value();
    x = 0;

    evaluation += expression.value() / 2.0;
    x += step_size;
    while (x < 1.0){
    ...
    evaluation += (val_in_1 + expression.value()) * (1.0 - x) / 2.0;
...
PYBIND11_MODULE(numIntTrapezoidQ, handle) {
    handle.doc() = "";
    handle.def("trapezoid_quadrature_01", &trapezoid_quadrature_01);
    handle.def("romberg_quadrature_01", &romberg_quadrature_01);
}