...
int main(){
    ...
    for (auto &test : TESTS_TO_RUN){
        ...
        double calculated = integrate_trapezoid(test.input_name, test.variable_name, a, b, h);
        double true_result = res_func(b) - res_func(a);

        printf("Całka numeryczna z funkcji:\t%s\nna przedziale (%f, %f) metodą trapezów \n\tPrawdziwy: %.12f\tObliczony: %.12f\n",
            test.input_name.c_str(),
            a, b,
            true_result, calculated
        );

        assert_printf(
            fabs(calculated - true_result) <= tol,
            "Dokładność [%e] nieosiągnięta.\n Poprawny: %.12f\tUzyskany: %.12f\n",
            tol, true_result, calculated
        );
    }
    ...
    for (auto &test : TESTS_TO_RUN){
        ...
        printf("Całka numeryczna z funkcji:\t%s\nna przedziale (%f, %f) metodą Romberga\n\tPrawdziwy: %.12f\tObliczony: %.12f\n",
            test.input_name.c_str(),
            a, b,
            true_result, calculated
        );
